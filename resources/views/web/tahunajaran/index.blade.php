                                        ...
                                        <td>
                                            <div class="form-switch-toggle">
                                                ...
                                                <input type="hidden" name="status_tahunAjaran"
                                                    id="status_tahunAjaran_hidden_{{ $tahunId }}"
                                                    value="{{ $tahun->status_tahunAjaran }}">

                                                <input type="checkbox" id="status_tahunAjaran_switch_{{ $tahunId }}"
                                                    class="switch-toggle tahunAjaran-toggle" data-id="{{ $tahunId }}"
                                                    {{ $tahun->status_tahunAjaran == 'aktif' ? 'checked' : '' }}>

                                                <label for="status_tahunAjaran_switch_{{ $tahunId }}"
                                                    class="switch-label"></label>
                                                <span
                                                    id="status_tahunAjaran_text_{{ $tahunId }}">{{ ucfirst($tahun->status_tahunAjaran) }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                                data-bs-target="#editTahunAjaranModal{{ $tahun->id_tahunAjaran }}"
                                                title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form action="{{ route('tahunajaran.destroy', $tahun->id_tahunAjaran) }}"
                                                method="POST" style="display:inline-block;" class="form-delete">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-danger btn-delete" type="button"
                                                    data-id="{{ $tahun->id_tahunAjaran }}" title="Hapus">
                    ...
                    if (result.isConfirmed) {
                        fetch(`/tahun-ajaran/${tahunId}/toggle-status`, {
                            ...
                            .then(async response => {
                                ...
                                if (!response.ok) {
                                    // Gagal, tampilkan pesan error dan reset toggle
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Gagal',
                                        text: data.message ||
                                    ...
                                    document.getElementById(
                                            `status_tahunAjaran_text_${tahunId}`)
                                        .textContent =
                                        newStatus.charAt(0).toUpperCase() + newStatus.slice(
                                            1);
