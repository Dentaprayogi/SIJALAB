                        ...
                        @else
                            @foreach($prodi as $prd)
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalEditProdi{{ $prd->id_prodi }}" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <form action="{{ route('prodi.destroy', $prd->id_prodi) }}" method="POST" style="display:inline-block;" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger btn-delete" type="button" data-id="{{ $prd->id_prodi }}" title="Hapus">
...
@include('web.prodi.create')
@include('web.prodi.edit')
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        @if ($errors->any())
            Swal.fire({
                ...
                let oldId = "{{ old('id_prodi') }}"; // Ambil id dari form edit yang gagal

                if (oldId) {
                    var editModal = new bootstrap.Modal(document.getElementById('modalEditProdi' + oldId));
                    editModal.show();
                } else {
                    var tambahModal = new bootstrap.Modal(document.getElementById('modalTambahProdi'));
                    tambahModal.show();
                }
            });
        @endif
