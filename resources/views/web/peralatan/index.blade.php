                        ...
                        @else
                            @foreach($peralatan as $peralatans)
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalEditPeralatan{{ $peralatans->id_peralatan }}" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <form action="{{ route('peralatan.destroy', $peralatans->id_peralatan) }}" method="POST" style="display:inline-block;" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger btn-delete" type="button" data-id="{{ $peralatans->id_peralatan }}" title="Hapus">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        @if ($errors->any())
            Swal.fire({
                ...
                if (oldId) {
                    var editModal = new bootstrap.Modal(document.getElementById('modalEditPeralatan' + oldId));
                    editModal.show();
                } else {
                    var tambahModal = new bootstrap.Modal(document.getElementById('modalTambahPeralatan'));
                    tambahModal.show();
                }
            });
        @endif
