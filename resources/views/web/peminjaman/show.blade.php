        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6 text-center">
                    @if ($peminjaman->user->mahasiswa->foto_ktm)
                        ...
                        <div class="mt-2">
                            <img src="{{ asset('storage/' . $peminjaman->user->mahasiswa->foto_ktm) }}"
                                 alt="Foto KTM"
                                 class="img-thumbnail"
            ...
            <div class="mt-4 d-flex justify-content-end gap-2">
                @if ($peminjaman->status_peminjaman === 'pengajuan')
                    <div>
                        <form action="{{ route('peminjaman.setujui', $peminjaman->id_peminjaman) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="btn btn-primary">Setujui</button>
                        </form>
                
                        <button type="button" class="btn btn-danger" onclick="showTolakModal({{ $peminjaman->id_peminjaman }})">Tolak</button>
                    </div>
                @elseif ($peminjaman->status_peminjaman === 'dipinjam')
                    <form action="{{ route('peminjaman.selesai', $peminjaman->id_peminjaman) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PUT')
