                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="form-switch-toggle">
                                            ...
                                            <input type="hidden" name="status_user" id="status_user_hidden_{{ $id }}" value="{{ $user->status_user }}">

                                            <input type="checkbox"
                                                id="status_user_switch_{{ $id }}"
                                                class="switch-toggle status-toggle"
                                                data-id="{{ $id }}"
                                                data-role="{{ $user->role }}"
                                                ...
                                                @endif
                                            >
                                            <label for="status_user_switch_{{ $id }}" class="switch-label"></label>
                                            <span id="status_text_{{ $id }}">{{ $user->status_user == 'aktif' ? 'Aktif' : 'Nonaktif' }}</span>
                                        </div>
                                    </td>                                    
                                    <td>
                                        <!-- Tombol Detail -->
                                        <a href="{{ route('users.show', $user->id) }}" class="btn btn-info" title="Lihat Detail">
                                            <i class="fas fa-eye"></i> <!-- Icon Detail -->
                                        </a>
                                        @if ($user->role === 'teknisi')
                                            <!-- Tombol Hapus (non-aktif) -->
                                            <button class="btn btn-secondary" type="button" title="Tidak bisa dihapus" style="cursor: not-allowed;">
            ...
            toggle.addEventListener('change', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById(`status_text_${userId}`).textContent = newStatus.charAt(0).toUpperCase() + newStatus.slice(1);
