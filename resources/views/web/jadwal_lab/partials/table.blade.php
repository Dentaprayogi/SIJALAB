        ...
        @else
            @foreach ($data as $item)
                <tr>
                    @auth
                        @if (Auth::user()->role === 'teknisi')
                            <td><input type="checkbox" class="select-item" name="selected[]"
                                    value="{{ $item->id_jadwalLab }}">
                            </td>
                        @endif
                    @endauth
                        ...
                        @auth
                            @if (Auth::user()->role === 'teknisi')
                                <div class="form-switch-toggle">
                                    ...
                                    <input type="hidden" name="status_jadwalLab"
                                        id="status_jadwalLab_hidden_{{ $id }}"
                                        value="{{ $item->status_jadwalLab }}">

                                    <input type="checkbox" id="status_jadwalLab_switch_{{ $id }}"
                                        class="switch-toggle jadwalLab-toggle" data-id="{{ $id }}"
                                        {{ $item->status_jadwalLab == 'aktif' ? 'checked' : '' }}>

                                    <label for="status_jadwalLab_switch_{{ $id }}" class="switch-label"></label>
                                    <span
                                        id="status_jadwalLab_text_{{ $id }}">{{ $item->status_jadwalLab == 'aktif' ? 'Aktif' : 'Nonaktif' }}</span>
                                </div>
                            @endif
                        @endauth
                        @auth
                            @if (Auth::user()->role === 'mahasiswa')
                                @php
                                    $badgeClass = match ($item->status_jadwalLab) {
                    ...
                    @auth
                        @if (Auth::user()->role === 'teknisi')
                            <td>
                                ...
                                <form action="{{ route('jadwal_lab.destroy', $item->id_jadwalLab) }}" method="POST"
                                    style="display:inline-block;" class="form-delete">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger btn-delete" type="button"
                                        data-id="{{ $item->id_jadwalLab }}" title="Hapus">
